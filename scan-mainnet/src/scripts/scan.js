...
export async function fetchTransactions(page = 1) {
    try {
        const response = await axios.get(`https://node-w3link-mainnet.azurewebsites.net/api/events?page=${page}`)
...
export async function fetchTransaction(hash) {
    try {
        const response = await axios.get(`https://node-w3link-mainnet.azurewebsites.net/api/events/${hash}`)
        return response.data
    } catch (error) {
        console.error(error)
        return []
