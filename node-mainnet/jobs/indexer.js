...
const { w3linkIds, Pego, rpcs } = require('../configs/chains.config')
...
const job = new CronJob('*/20 * * * * *', async function () {
    try {
        ...
        const fromBlock = json.startBlocks[Pego]

        console.log(`Indexer: Running Job from ${fromBlock}`)

        const web3 = new Web3(rpcs[Pego])
        const w3link = new web3.eth.Contract(W3Link.abi, w3linkIds[Pego])
